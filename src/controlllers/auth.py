...
class AuthController:
    ...
    async def login(self, user_session_id: str, phone: str) -> None:
        try:
            otp_code = await OTPHandler(redis_db=self.redis_db).send_otp_verify_code(phone=phone)
            registration_data = {
                "phone": phone,
                "otp_code": otp_code,
            }
    ...
    async def verify_login(self, db_session: AsyncSession, otp_code, user_session_id: str) -> Token:
        try:
            ...
            is_valid = await OTPHandler(redis_db=self.redis_db).validate_otp_code(phone=phone, otp_code=otp_code)
            if is_valid:
